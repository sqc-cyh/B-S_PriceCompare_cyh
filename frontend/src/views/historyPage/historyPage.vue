...
const query = route.query.searchQuery;
...
const fetchHistoricalPrices = async () => {
  ...
  try {
    ...
    // 按日期排序
    historicalPrices.value.sort((a, b) => new Date(a.date) - new Date(b.date));
...
// 返回搜索结果页面的函数
const goBack = () => {
  router.push({ path: '/searchresults', query: { from: 'history', q: query } });
}

onMounted(() => {
  console.log(store.needSearch_suning)
  console.log('jingdong:', store.needSearch_jingdong)
  // 设置 localStorage 中的 isPageActive 变量为 true
  localStorage.setItem('isPageActive', 'true');
  
  fetchHistoricalPrices() // 组件挂载时获取数据
  if(store.getSearchResults_suning() !== null) store.needSearch_suning = false;
  if(store.getSearchResults_jingdong() !== null) store.needSearch_jingdong = false;
  console.log('next:', store.needSearch_suning)
  console.log('jingdong:', store.needSearch_jingdong)
  console.log(store.getSearchResults_suning())
})
</script>
