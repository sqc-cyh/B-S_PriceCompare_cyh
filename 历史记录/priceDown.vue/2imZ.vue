<template>
  ...
  export default {
    name: 'PriceDownPage',
    setup() {
      const priceDownItems = ref([]);
  
      const fetchPriceDownItems = async (username) => {
        try {
          const response = await axios.post('http://127.0.0.1:8000/users/search_price_alert/', {
            username: username,
          });
          priceDownItems.value = response.data; 
        } catch (error) {
          console.error('获取降价提醒商品失败:', error);
          ElMessage.error('无法获取降价提醒商品');
        }
      };
  
      const removePriceAlert = async (imageUrl) => {
        const username = localStorage.getItem('username');
        try {
          await axios.post('http://127.0.0.1:8000/users/remove_price_alert/', {
            imageUrl: imageUrl,
            username: username,
          });
  
          // 更新本地数据
          priceDownItems.value = priceDownItems.value.filter(item => item.imageUrl !== imageUrl);
          ElMessage.success('已取消降价提醒');
        } catch (error) {
          console.error('取消降价提醒失败:', error);
          ElMessage.error('取消降价提醒失败');
        }
      };
      ...
      onMounted(() => {
        const username = localStorage.getItem('username');
        if (username) {
          fetchPriceDownItems(username); // 在组件挂载后调用 API
        } else {
