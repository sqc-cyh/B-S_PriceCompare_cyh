  ...
  export default {
    ...
    setup() {
      ...
      const sendCaptcha = async () => {
        try {
          const response = await axios.post('http://127.0.0.1:8000/myapp/users/send-captcha/', {
          email: form.email
        });
      ...
      const submitForm = async () => {
        formRef.value.validate(async (valid) => {
          if (valid) {
            ...
            try {
              // 验证验证码
              const response = await axios.post('http://127.0.0.1:8000/myapp/users/verify-captcha/', {
