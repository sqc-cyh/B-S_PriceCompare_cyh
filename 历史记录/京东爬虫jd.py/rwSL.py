...
# 开始爬取多页数据
def get_more(web, page_num):
    titless, prices, saleses, shop_names, urls = get_product(web)
    for i in range(1):
        print(f"爬取第{i}页数据")
...
def main():
    ...
    get_more(web, page_num)
    data = {"标题": titless, '价格': prices, "店铺": shop_names, "销量": saleses, "图片": urls}
    pd.DataFrame(data).to_csv(f'./data/京东{key}销售.csv', index=False)
    print("全部完成")
    web.quit()
