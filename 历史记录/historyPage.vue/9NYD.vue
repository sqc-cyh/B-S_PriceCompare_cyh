...
export default {
  ...
  setup() {
    ...
    const chartContainer = ref(null); // 修改了变量名以避免与echarts实例混淆
    const chartData = ref({
      labels: [],
      prices: [],
    });
    ...
    const fetchHistoryData = () => {
      ...
      loading.value = false; // 确保数据加载完毕后再设置loading为false
    };
    ...
    onMounted(() => {
      ...
      watch(chartData, () => {
        nextTick(() => { // 使用nextTick确保DOM更新后渲染图表
          renderChart();
        });
      });
    });
    ...
    return {
      loading,
      chartContainer, // 修改了变量名以避免与echarts实例混淆
      chartData,
    };
  },
};
