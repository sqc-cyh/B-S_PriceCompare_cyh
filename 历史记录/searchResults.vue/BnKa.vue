    ...
    setup() {
      ...
      const fetchSearchResults = async () => {
        ...
        try {
          ...
          // Fetch results from the selected platform
          ...
          if (response.data.results) {
            if (selectedPlatform.value === 'suning') {
              ...
              localStorage.setItem(`results_suning_${searchQuery.value}`, JSON.stringify(suningResults.value)); // 缓存结果
            } else if (selectedPlatform.value === 'jd') {
              jdResults.value = response.data.results;
              localStorage.setItem(`results_jd_${searchQuery.value}`, JSON.stringify(jdResults.value)); // 缓存结果
            } else {
              tbResults.value = response.data.results;
              localStorage.setItem(`results_taobao_${searchQuery.value}`, JSON.stringify(tbResults.value)); // 缓存结果
            }
          }
      ...
      const selectPlatform = (platform) => {
        if (platform !== selectedPlatform.value) {
          ...
          fetchSearchResults(); // 每次切换平台时重新获取结果
        }
      }
  
      const goToHistory = (image) => {
        router.push({ path: '/history', query: { image } })  // 路由跳转并传递参数
      }
  
      onMounted(() => {
        // 如果是从 /history 路由返回，加载缓存
        if (route.query.from === 'history') {
          ...
          if (cachedResults) {
            ...
            fetchSearchResults(); // 如果没有缓存，重新搜索
          }
        } else {
          fetchSearchResults(); // 其他情况直接搜索
        }
      })
