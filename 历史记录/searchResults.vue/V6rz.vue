  ...
  <div class="search-results-container">
    <el-card class="search-card">
      ...
      <div class="back-button-container">
        <el-button type="info" @click="goToCompare" class="back-button">返回对比页面</el-button>
      </div>
    </el-card>
  </div>
</template>
  ...
  setup() {
    ...
    const fetchSearchResults = async () => {
      ...
      try {
        ...
        if (store.needSearch_suning === true && selectedPlatform.value === 'suning') { 
          console.log('suning true sou')
          ...
          if (response.data.results) {
            ...
            store.setSearchResults_suning(suningResults.value)
            store.needSearch_suning = false;
            localStorage.setItem(`results_suning_${searchQuery.value}`, JSON.stringify(suningResults.value)); 
          }
        } else if (store.needSearch_jingdong === true && selectedPlatform.value === 'jd') {
          console.log('jingdong true sou')
          const response = await axios.post(`http://127.0.0.1:8000/users/search_${selectedPlatform.value}/`, {
            query: searchQuery.value,
            platform: selectedPlatform.value,
          });
          if (response.data.results) {
            ...
            store.setSearchResults_jingdong(jdResults.value)
            store.needSearch_jingdong = false;
            localStorage.setItem(`results_jd_${searchQuery.value}`, JSON.stringify(jdResults.value));
          }
        } else if (store.needSearch_jingdong === false || store.needSearch_suning === false) {
          if (store.SearchResults_suning === null) {
            console.log('suning null but sou')
            const response = await axios.post(`http://127.0.0.1:8000/users/search_suning/`, {
              query: searchQuery.value,
              platform: 'suning',
            });
            suningResults.value = response.data.results
            ...
            suningResults.value = store.getSearchResults_suning();
          }
          if (store.SearchResults_jingdong === null) {
            console.log('jingdong null but sou')
            const response = await axios.post(`http://127.0.0.1:8000/users/search_jd/`, {
              query: searchQuery.value,
              platform: 'jd',
            });
            jdResults.value = response.data.results
            ...
            jdResults.value = store.getSearchResults_jingdong();
          }
        }
      } catch (error) {
    ...
    onMounted(() => {
      console.log(store.needSearch_jingdong)
