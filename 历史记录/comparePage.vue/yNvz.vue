...
const performSearch = () => {
  console.log('Searching for:', form.value.searchQuery);
  showSearchSuggestions.value = false;
  // Navigate to the search results page with the search query
  const router = useRouter();
  router.push({ name: 'SearchResults', query: { q: form.value.searchQuery } }); // 确保名称匹配
};

const selectSearchItem = (item) => {
  form.value.searchQuery = item;
  performSearch();
};

const navigateToAccount = () => {
  const router = useRouter();
  router.push('/user'); // 跳转到 /user 路由
};

const navigateToPriceDown = () => {
  const router = useRouter();
  router.push('/price_down'); // 确保名称与路由配置匹配
};
</script>
<script>
export default {
  name: 'comparePage' // 这里为组件命名为 'ComparePage'
};
</script>
<style scoped>
.home-container {
