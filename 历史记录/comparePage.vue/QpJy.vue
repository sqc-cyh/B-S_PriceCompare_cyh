...
export default {
  ...
  setup() {
    ...
    const checkForPriceDown = async (username) => {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/has_pricedown/?username=${username}`);
          const hasPriceDown = response.data.hasPriceDown; // 假设后端返回的字段为 hasPriceDown
          if (hasPriceDown) {
            ElMessage.success('存在降价商品');
            // 这里可以进行进一步的处理，例如更新状态或跳转页面
          } else {
            ElMessage.info('没有降价商品');
          }
        } catch (error) {
          ElMessage.error('检查降价时发生错误');
          console.error('检查降价时发生错误:', error);
        }
    };
    onMounted(async () => {
