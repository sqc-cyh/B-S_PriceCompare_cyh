...
* 爬虫可能出现的问题：
    * 在完成项目的过程中，京东更新了多次网站，例如一开始京东没有覆盖翻页按钮，后来覆盖了，因此需要修改爬虫函数以适配新的版本，目前的爬虫函数只能满足12.24号时的版本。
    * 京东爬虫可能会出现如下报错：`selenium.common.exceptions.InvalidCookieDomainException: Message: invalid cookie domain: Cookie 'domain' mismatch`，这是因为京东可能会进行cookie的更新，我的cookie是使用手机号登录获取的，理论上可以维持一个月。                             
## 附录
